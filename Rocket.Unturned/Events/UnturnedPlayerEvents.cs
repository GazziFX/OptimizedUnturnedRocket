    ...
    public sealed class UnturnedPlayerEvents : UnturnedPlayerComponent
    {
        ...
        internal static void TriggerSend(SteamPlayer s, string W, ESteamCall X, ESteamPacket l, params object[] R)
        {
            try
            {
                ...
                {
                    OnPlayerWear.TryInvoke(rp, Enum.Parse(typeof(Wearables), W.Substring(8)), (ushort)R[0], R.Length > 1 ? (byte?)R[1] : null);
                    return;
                }
                switch (W)
                {
                    ...
                    case "tellStance":
                        ...
                        break;
                    case "tellGesture":
                        OnPlayerUpdateGesture.TryInvoke(rp, (PlayerGesture)(byte)R[0]);
                        instance.OnUpdateGesture.TryInvoke( rp, (PlayerGesture)(byte)R[0]);
                        break;
                    case "tellStat":
                    ...
                    case "tellDead":
                        ...
                        break;
                    case "tellDeath":
                        OnPlayerDeath.TryInvoke(rp, (EDeathCause)(byte)R[0], (ELimb)(byte)R[1], (CSteamID)R[2]);
                        instance.OnDeath.TryInvoke(rp, (EDeathCause)(byte)R[0], (ELimb)(byte)R[1], (CSteamID)R[2]);
                        break;
                    default:
#if DEBUG
        ...
        {
            OnPlayerInventoryResized.TryInvoke(Player, (InventoryGroup)E, M, U);
            OnInventoryResized.TryInvoke(Player, (InventoryGroup)E, M, U);
        }
        ...
        {
            OnPlayerInventoryRemoved.TryInvoke(Player, (InventoryGroup)E, y, f);
            OnInventoryRemoved.TryInvoke(Player, (InventoryGroup)E, y, f);
        }
        ...
        {
            OnPlayerInventoryAdded.TryInvoke(Player,(InventoryGroup)E, u, J);
            OnInventoryAdded.TryInvoke(Player, (InventoryGroup)E, u, J);
        }
        ...
        internal static Color firePlayerChatted(UnturnedPlayer player, EChatMode chatMode, Color color, string msg, ref bool cancel)
        {
            ...
            {
                foreach (var handler in OnPlayerChatted.GetInvocationList())
                {
                    try
                    {
                        ((PlayerChatted)handler)(player, ref color, msg, chatMode, ref cancel);
                    }
                    catch (Exception ex)
                    {
