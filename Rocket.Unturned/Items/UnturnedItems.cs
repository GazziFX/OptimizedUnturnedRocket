...
namespace Rocket.Unturned.Items
{
    ...
    public static class UnturnedItems
    {
        public static ItemAsset GetItemAssetByName(string name)
        {
            ...
            foreach (var item in SDG.Unturned.Assets.find(EAssetType.ITEM))
            {
                var asset = (ItemAsset)item;
                if (!string.IsNullOrEmpty(asset.itemName) && asset.itemName.IndexOf(name, StringComparison.OrdinalIgnoreCase) != -1)
                    return asset;
            }
            return null;
